 ...
 * @author Belleve(belleve@typeof.net)
 */
        ...
        function prepareDump(ttf) {
            ...
            if (this.options.writeUnknownTables) {
                for(var tableName in ttf) if(ttf[tableName] && ttf[tableName].name === tableName) {
                    tables.push(tableName)
                }
            }

            ttf.writeOptions.hinting = !!this.options.hinting;
            ttf.writeOptions.preserveOS2Version = !!this.options.preserveOS2Version
                ...
                if (glyf.unicode) {
                    ...
                    glyf.unicode.forEach(function (u) {
                        if (checkUnicodeRepeat[u]) {
                            error.raise(10200, u.toString(16));
                        }
                        else {
        ...
        var UnknownTableFormat = function(tableName){
            return function(){
                this.size = function(ttf){
                    return ttf[tableName] ? ttf[tableName].content.length : 0
                }
                this.write = function(writer, ttf){
                    if(ttf[tableName]) {
                        writer.writeBytes(ttf[tableName].content, ttf[tableName].content.length)
                    }
                }
            }
        }
        ...
        function write(ttf) {
            ...
            ttf.writeOptions.tables.forEach(function (tableName) {
                var offset = ttfSize;
                var Writer = supportTables[tableName] || UnknownTableFormat(tableName);
                var tableSize = new Writer().size(ttf); // 原始的表大小
            ...
            ttf.support.tables.forEach(function (table) {
                ...
                !new table.Writer().write(writer, ttf);

                if (table.size > table.length) {
                    // 对齐字节
                    writer.writeEmpty(table.size - table.length);
                }
        ...
        function TTFWriter(options) {
            ...
            this.options = {
                ...
                writeUnknownTables: options.writeUnknownTables || false, // Drop unknown tables
                preserveOS2Version: options.preserveOS2Version || false, // Preserve OS2 version before writing
                preserveXAvgCharWidth: options.preserveXAvgCharWidth || false // Preserve average character width, necessary for making CSRSS-compatible fonts.
            };
        }
