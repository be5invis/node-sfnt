 ...
 */
        ...
        function readSubTable(reader, ttf, subTable, cmapOffset) {
            ...
            // UVS table format. Read default layout only.
            else if (subTable.format === 14 && false) {
                var format14 = subTable;
                
                format14.length = reader.readUint32();
                var numVarSelectorRecords = reader.readUint32();
                format14.variants = [];
                for(var variant = 0; variant < format14.numVarSelectorRecords; variant ++) {
                    var variantRecord = {
                        varSelector: reader.readByte() * 65536 + reader.readByte() * 256 + reader.readByte(),
                        defaultUVSOffset: reader.readUint32(),
                        nonDefaultUVSOffset: reader.readUint32()
                    };
                    format14.variants.push(variantRecord);
                }
            }
            else {
